...
词嵌入空间中相近的词应该是相关的(mutually related)
...
单词频率和k-近邻实验中排序的位置有关，高频的词更可能靠前，所以余弦相似度作为内部评价的方法会受到单词频率的影响

总结：

1. 内部评价(intrinsic evaluations)通过分析单词语义和词向量几何间的联系来评估词向量的质量，大部分的评价方法都使用预先收集好的**词对**(precollected inventories)来计算结果。本文部分实验使用的单词对是通过词嵌入算法k-近邻得到的，在此基础上人为评估
2. 词的频率会影响余弦相似度的计算，甚至会影响k-近邻的排序结果，频率高的单词，与query词匹配的概率会高一点，所以排序会靠前
3. 词嵌入模型会编码一点的词频信息，并且编码方式是平滑的
4. 在词嵌入空间中coherence和relatedness任务一点关系，毕竟空间中离的近的单词在语义上会相关，并且都是有联系的